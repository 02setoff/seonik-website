    ...
    <div style={{ maxWidth: "800px", margin: "0 auto", padding: "clamp(40px,8vw,64px) clamp(20px,5vw,40px) 96px" }}>
        ...
        {NAV.map((item) => (
          <Link key={item.href} href={item.href}
            style={{
              ...
              color: item.href === "/about/vision" ? "var(--text-primary)" : "var(--text-placeholder)",
              fontWeight: item.href === "/about/vision" ? 700 : 400,
              textDecoration: "none",
              borderBottom: item.href === "/about/vision" ? "2px solid var(--text-primary)" : "2px solid transparent",
      ...
      <div style={{ marginBottom: "48px" }}>
        ...
        <div style={{ padding: "24px 28px", borderLeft: "3px solid var(--text-primary)", backgroundColor: "var(--bg-subtle)" }}>
          <p style={{ fontSize: "14px", fontFamily: "'Pretendard', sans-serif", color: "var(--text-secondary)", lineHeight: "1.9", margin: 0 }}>
            우리는 세상의 모든 정보를 다 다루지 않습니다. <strong style={{ color: "var(--text-primary)" }}>&ldquo;창업·마이크로 SaaS·AI 비즈니스&rdquo;</strong>라는 뾰족한 니치(Niche)에 집중하여, 대형 기관보다 훨씬 빠르게 글로벌 소스를 모니터링하고 번역하며 패턴을 포착합니다. AI 에이전트를 활용한 고속 분석으로 실행 가능한 인텔리전스를 기존 기관과는 비교할 수 없는 속도로 제공합니다.
          </p>
        </div>
      </div>
      ...
      <div style={{ marginBottom: "48px" }}>
        ...
        <p style={{ fontSize: "15px", fontFamily: "'Pretendard', sans-serif", color: "var(--text-secondary)", lineHeight: "1.9", marginBottom: "16px" }}>
          선익은 기존 시장에서 점유율을 다투는 경쟁을 하지 않습니다. 현재 한국 시장에는 <strong style={{ color: "var(--text-primary)" }}>1인 사업자와 중소기업에 특화된 정보기관</strong> 형태의 플레이어가 사실상 존재하지 않습니다.
        </p>
        <p style={{ fontSize: "15px", fontFamily: "'Pretendard', sans-serif", color: "var(--text-secondary)", lineHeight: "1.9" }}>
          우리는 기존의 &ldquo;비즈니스 뉴스레터&rdquo;나 &ldquo;창업 커뮤니티&rdquo;의 틀을 부수고, <strong style={{ color: "var(--text-primary)" }}>&ldquo;1인 사업자와 중소기업을 위한 민간 싱크탱크&rdquo;</strong>라는 독보적인 카테고리를 창조합니다. &ldquo;제1의&rdquo;라는 선언은 이 새로운 카테고리를 우리가 정의하고 이끌어가겠다는 카테고리 킹(Category King)으로서의 선언입니다.
        </p>
      </div>
          ...
          {[
            ...
            <div key={item.phase} style={{ display: "flex", gap: "20px" }}>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", minWidth: "2px" }}>
                <div style={{ width: "10px", height: "10px", borderRadius: "50%", backgroundColor: "var(--text-primary)", flexShrink: 0, marginTop: "22px" }} />
                {i < arr.length - 1 && <div style={{ width: "2px", flex: 1, backgroundColor: "var(--border)", minHeight: "32px" }} />}
              </div>
      ...
      <div style={{ borderTop: "1px solid var(--border-light)", paddingTop: "32px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <Link href="/about/mission" style={{ fontSize: "13px", color: "var(--text-muted)", fontFamily: "'Pretendard', sans-serif", textDecoration: "none" }}>← 미션</Link>
        <p style={{ fontSize: "12px", color: "var(--text-placeholder)", fontFamily: "Inter, sans-serif" }}>先益 — 앞서나가는 정보로 실행가들을 이롭게</p>
        <Link href="/about/company" style={{ fontSize: "13px", color: "var(--text-primary)", fontFamily: "'Pretendard', sans-serif", textDecoration: "none", fontWeight: 600 }}>회사명 →</Link>
      </div>
    </div>
  );
}
